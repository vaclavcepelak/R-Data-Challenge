...
p <- p + geom_freqpoly(aes(weight = Weight), bins = 30) 
...
p <- p + theme(legend.title=element_blank(),
               ...
               text = element_text(family = 'Lato', colour = 'grey')) 
...
```{r spots, message = FALSE, fig.width = 8}
# load map
...
# load the map
map <- ggmap(karlin_map, extent = "panel", darken = c(0.3, colours[1]))
# add points
...
```{r hashtags, message = FALSE, fig.width = 8}
...
```
<font size = '15'>hashtag minimum frequency: <b>`r unique(hashtags_freq[freq_rank == top_n, N])`</b></font>


## Hashtags are used at specific spots

```{r corresp, message = FALSE, fig.width = 8}
...
```{r cluster_data, message = FALSE, fig.width = 8}

# get data for clustering
locations_cluster <- locations[n_posts >= min_n_location, 
                               .(location_id, location_name, time_peak,
...
```{r hcluster, message = FALSE, fig.width = 8}
...
- We can see cca 4 groups in the dendrogram (with some individual cases leaving on higher height)
- We cut the dendrogram on the **height = `cut_h`** and all the **other** groups recode together

```{r cluster_results, message = FALSE, fig.width = 8}
...
assign_tab <- assign_tab[, .(c40 = as.numeric(V1), 
                             c50 = as.numeric(V2), 
                             cluster)]

locations_cluster[, c40 := assign_cl[, '40']]
locations_cluster[, c50 := assign_cl[, '50']]
setkey(locations_cluster, c40, c50)
setkey(assign_tab, c40, c50)
...
cluster_results <- data.table::melt(locations_cluster_scaled[, !c('c40', 'c50'),
                                                             with = FALSE], 
                                    id.vars = c('location_id', 'location_name',
                                                'cluster'))
...
cl_sum[, `Peak time` := as.POSIXct(time_peak, origin = '1970-01-01')]
cl_sum[, `% of posts on a free day` := paste0(round(f_posts_freeday * 100, 1),
                                              '%')]
cl_sum[, `N likes per post` := round(n_likes_per_post, 1)]
cl_sum[, `Top 5 hashtags` := ht_top5$ht_top5]
cl_sum[, c('time_peak', 'f_posts_freeday', 'n_likes_per_post') := NULL, 
       with = FALSE]
...
```{r clusters_map, message = FALSE, fig.width = 8}
